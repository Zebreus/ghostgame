...
const char SWITCHES[8] = {2,4,7,8,12,A0,A1,A2};
const char LEDS[8] = {6,5,9,3,10,11,13,A3};
...
void loop() {
  ...
  for(int i = 0;i<animationFill;i++){
    ...
    digitalWrite(animationQueue[i].port,!animationQueue[i].status);
...
void setPower(char mpower){
  ...
  fill(power,millis()+SET_POWER*0.5);
...
void newGame(char mpower){
  ...
  fill(power,millis()+SET_POWER*0.6);
  fill(power,millis()+SET_POWER);
}
...
void fill(int amount,unsigned long time){
  for(int i = 0;i<amount;i++){
    addToAnimationQueue(LEDS[i],1,time);
  }
